...
int main()
{
	...
	controller_findLastId("MOCK_DATA.csv", "LastId.bin");
	do
	{
		puts("----------------------------------------------------------------------------------------------------");
				...
				"9)Guardar los datos de los pasajeros en el archivo data.csv (modo binario).\n10)Salir\nOpcion: ", 3, 0, 11, "\nNUMERO NO VALID0\n", &opcionMenu))
		{
			...
			case 1:
				if(banderaCargaText)
				{
					if(controller_loadFromText("MOCK_DATA.csv", listaPasajeros))
					{
			...
			case 2:
				if(banderaCargaBin)
				{
					if(controller_loadFromBinary("DataPassengersBin.bin", listaPasajeros))
					{
						puts("No se pudo Cargar Datos.");
					}
					else
			...
			case 9:
				...
				controller_saveAsBinary("DataPassengersBin.bin",listaPasajeros);
				banderaGuardado = 0;
				break;
			case 10:
				if(banderaGuardado)
